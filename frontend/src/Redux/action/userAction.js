...
export const userDetailsById = (id) => async (dispatch, getState) => {
  try {
    dispatch({
      type: userTypes.FETCH_USERDETAILS_REQUEST,
    });
    const {
      userLogin: { userInfo },
    } = getState();
    ...
    const res = await fetch(`/api/v1/admin/getUserById/${id}`, options);
    const data = await res.json();
   

    dispatch({
      type: userTypes.FETCH_USERDETAILS_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type: userTypes.FETCH_USERDETAILS_FAILURE,
      payload: error,
    });
  }
};
